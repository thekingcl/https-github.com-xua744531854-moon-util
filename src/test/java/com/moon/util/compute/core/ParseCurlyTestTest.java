...
class ParseCurlyTestTest {
    ...
    static AsHandler running0(String expression) {
    ...
    void doVarList() {
        data = running0("{key: 'value', null: (50 + 60   ), true: @DateUtil.now()+0}").use();

        str = "{key, key,true,[true],[null]}";
        handler = running0(str);
        assertions.assertThrows(() -> handler.use());
        res = handler.use(data);
